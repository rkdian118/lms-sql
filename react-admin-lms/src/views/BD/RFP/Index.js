...
function Row({ row, key, currentPage, pageLength, search, rfpStatusId }) {
    ...
    const handleCloseModel = () => {
        ...
        setOpenStatus(false);
        setId('');
        dispatch(GetAllRfpRequestApi(newCurrentPage, pageLength, search, rfpStatusId));
...
const Index = () => {
    ...
    useEffect(() => {
        dispatch(GetAllRfpRequestApi(1, pageLength, '', rfpStatusId));
    }, [pageLength]);
    ...
    const handleCloseModel = () => {
        setOpenModel(false);
        dispatch(GetAllRfpRequestApi(currentPage + 1, pageLength, search, rfpStatusId));
    };
    const onSearchLead = (e) => {
        if (e.target.value.length > 2) {
            dispatch(GetAllRfpRequestApi(currentPage + 1, pageLength, e.target.value, rfpStatusId));
        }
        if (e.target.value.length === 0) {
            dispatch(GetAllRfpRequestApi(currentPage + 1, pageLength, '', rfpStatusId));
        }
    ...
    const handleChangePage = (event, newPage) => {
        ...
        dispatch(GetAllRfpRequestApi(newPage + 1, pageLength, '', rfpStatusId));
    };

    const handleChangeRowsPerPage = (event) => {
        dispatch(GetAllRfpRequestApi(currentPage + 1, event.target.value, '', rfpStatusId));
    ...
    const onRfpStatusChange = (e, newValue) => {
        ...
        dispatch(GetAllRfpRequestApi(currentPage + 1, pageLength, '', selectValue));
    };
    return (
        <MainCard
            content={false}
