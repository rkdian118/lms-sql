...
import { ClearCallActivityData, GetAllCallsApi, GetAllLeadDropDownApi, GetCallActivityDetailsApi } from 'store/slices/businessAction';
...
function Row({ row, key, currentPage, pageLength, search, leadId, callStatusId }) {
    ...
    const handleCloseModel = () => {
        ...
        setId('');
        dispatch(GetAllCallsApi(newCurrentPage, pageLength, search, leadId, callStatusId));
    };
...
const Index = () => {
    ...
    useEffect(() => {
        dispatch(GetAllCallsApi(1, pageLength, search, leadId, callStatusId));
    }, [pageLength]);
    ...
    const handleCloseModel = () => {
        setOpenModel(false);
        dispatch(GetAllCallsApi(currentPage + 1, pageLength, search, leadId, callStatusId));
    };
    ...
    const onLeadChange = (e, newValue) => {
        ...
        dispatch(GetAllCallsApi(currentPage + 1, pageLength, search, selectValue, callStatusId));
    };

    const onCallStatusChange = (e, newValue) => {
        const selectValue = newValue ? newValue._id : '';
        setCallStatusId(selectValue);
        dispatch(GetAllCallsApi(currentPage + 1, pageLength, search, leadId, selectValue));
    };
    // console.log('🚀leadId:', leadId);
    const handleChangePage = (event, newPage) => {
        // console.log('🚀 newPage:', newPage);
        setCurrentPage(newPage);
        dispatch(GetAllCallsApi(newPage + 1, pageLength, search, leadId, callStatusId));
    };

    const handleChangeRowsPerPage = (event) => {
        dispatch(GetAllCallsApi(currentPage + 1, event.target.value, search, leadId, callStatusId));
        setPageLength(parseInt(event?.target.value));
        // setCurrentPage(newPage);
    };
    const onSearchLead = (e) => {
        if (e.target.value.length > 2) {
            dispatch(GetAllCallsApi(currentPage + 1, pageLength, e.target.value, leadId, callStatusId));
        }
        if (e.target.value.length === 0) {
            dispatch(GetAllCallsApi(currentPage + 1, pageLength, '', leadId, callStatusId));
        }
        setSearch(e.target.value);
