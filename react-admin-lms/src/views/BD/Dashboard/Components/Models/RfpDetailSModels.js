...
import { GetRfpDetailsDataApi } from 'store/slices/businessAction';
...
const RfpDetailsModels = (props) => {
    const { open, close, leadId, GetRfpDetailsDataApi } = props;
    // console.log('🚀  leadId:', leadId);
    const dispatch = useDispatch();
    const fileRef = useRef(null);
    // const restro = useSelector((state) => state.restroCat);
    const { getRfpDetailsData, getRfpDetailsLoading } = useSelector((state) => state.businessAction);
    // const { getBulkRestraurant } = restro;
    const [loader, setLoader] = useState(false);
    const [value, setValue] = useState('');

    useEffect(() => {
        GetRfpDetailsDataApi(leadId);
    }, [leadId]);
    ...
    return (
        <div>
            ...
            >
                ...
                >
                    <DialogContent dividers sx={{ m: 2, mt: 1, p: 0 }}>
                                            ...
                                            {getRfpDetailsLoading === true ? (
                                                <Grid item xs={12} sx={{ '&.MuiGrid-root': { pl: 0 } }}>
                                                    {/* <Box
                                                            ...
                                                            > */}
                                                    <Skeleton variant="rectangular" width="100%" height={20} sx={{ my: 1, mx: 0.5 }} />
                                                    {/* </Box> */}
                                                </Grid>
                                            ) : (
                                                <Grid item xs={12} sx={{ '&.MuiGrid-root': { pl: 0 } }}>
...
export default connect(null, { GetRfpDetailsDataApi })(RfpDetailsModels);
