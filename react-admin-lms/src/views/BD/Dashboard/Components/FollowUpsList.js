...
function Row({ row, key, currentPage, pageLength, compareDate, startDate, endDate }) {
    ...
    const handleLeadStatusUpdateModel = (data, type) => {
        ...
        if (type === 2) {
            const updateValue = data?.leadStatusData?.status_name === 'Hot Prospect';
                ...
                <TableCell align="left" sx={{ py: 0 }}>
                    {row?.projectionStatusData?.status_name ? (
                        <Chip
                            ...
                            sx={{
                                cursor:
                                    row?.leadStatusData?.status_name === 'Hot Prospect' ? 'pointer !important' : 'not-allowed !important',
                                fontSize: '0.875rem',
...
const FollowUpsList = ({ startDate = '', endDate = '' }) => {
    ...
    const handleChangeRowsPerPage = async (event) => {
        await GetAllFollowUpsDataApi(startDate, endDate, currentPage + 1, event?.target.value);
