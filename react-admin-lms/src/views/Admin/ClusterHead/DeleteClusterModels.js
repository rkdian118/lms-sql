...
import { ClusterGetApi, ClusterStatusChangeApi } from 'store/slices/adminAction';
...
const DeleteClusterModels = ({ open, close, deleteData, ClusterStatusChangeApi }) => {
    ...
    const onDeleteStatus = () => {
        setIsLoading(true);
        const newData = { cluster_id: deleteData._id, status: '3' };
        ClusterStatusChangeApi(newData)
            .then((res) => {
                setIsLoading(false);
                close();
                if (res.succeeded === true) {
                    dispatch(ClusterGetApi(1, 100));
                    dispatch(
                        openSnackbar({
                            open: true,
...
export default connect(null, { ClusterStatusChangeApi })(DeleteClusterModels);
