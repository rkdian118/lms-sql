...
import { ProposalTypeGetApi } from 'store/slices/masterAction';
...
const ProposalList = ({ ProposalTypeGetApi }) => {
    const { proposalTypeList, proposalTypeLoading } = useSelector((state) => state.masterAction);
    ...
    return (
        <>
            ...
            {!proposalTypeLoading && proposalTypeList.length > 0 ? (
            ...
            {openModel && <AddProposal open={openModel} close={() => handleCloseModel()} />}
            {id && openModel && <EditProposal open={openModel} close={() => handleCloseModel()} id={id} getName={name} />}
        </>
    );
};

export default connect(null, { ProposalTypeGetApi })(ProposalList);
