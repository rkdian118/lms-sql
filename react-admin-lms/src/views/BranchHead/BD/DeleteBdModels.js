...
const DeleteBdModels = ({ open, close, deleteData, BdStatusChangeApi }) => {
    ...
    const onDeleteStatus = () => {
        setIsLoading(true);
        const newData = { bd_id: deleteData._id, status: '3' };
        BdStatusChangeApi(newData)
            .then((res) => {
                setIsLoading(false);
                close();
                if (res.succeeded === true) {
                    dispatch(BDGetApi(1, 100));
                    dispatch(
                        openSnackbar({
                            open: true,
...
export default connect(null, { BdStatusChangeApi })(DeleteBdModels);
