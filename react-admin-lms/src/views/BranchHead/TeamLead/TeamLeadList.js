...
import { TeamLeadGetApi, TeamLeadStatusChangeApi } from 'store/slices/clusterAction';
...
import AddTeamLeadModels from './AddTeamLeadModels';
import EditTeamLeadModels from './EditTeamLeadModels';
import { openSnackbar } from 'store/slices/snackbar';
import DeleteTeamLeadModels from './DeleteTeamLeadModels';
import MuiTooltip from '@mui/material/Tooltip';

const TeamLeadList = ({ TeamLeadStatusChangeApi }) => {
    const newmenu = useSelector((state) => state.clusterAction);
    const { teamLeadList, teamLeadLoading } = newmenu;
    ...
    useEffect(() => {
        dispatch(TeamLeadGetApi(currentPage, pageLength));
    }, [currentPage, pageLength]);

    const onPaginationHandle = async (e, newPage) => {
        setCurrentPage(newPage);
        dispatch(TeamLeadGetApi(newPage, pageLength));
    };
    const onChangeStatus = (data) => {
        const newData = { manager_id: data._id, status: data.status === '1' ? '2' : '1' };
        TeamLeadStatusChangeApi(newData).then((res) => {
            ...
            <Grid container spacing={1}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    >
                        ...
                        {!teamLeadLoading && teamLeadList.docs.length > 0 ? (
                            teamLeadList.docs.map((list, i) => (
                                <Grid
                                    container
                                    spacing={0}
                ...
                {!teamLeadLoading && teamLeadList.totalDocs > 10 ? (
...
export default connect(null, { TeamLeadGetApi, TeamLeadStatusChangeApi })(TeamLeadList);
