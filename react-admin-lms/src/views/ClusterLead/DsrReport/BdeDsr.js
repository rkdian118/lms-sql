...
function Row({ row, key, currentPage, pageLength, leadStatus }) {
    ...
    const handleCloseModel = () => {
        setId('');
        dispatch(GetAllDSRApi(newCurrentPage, pageLength, '', leadStatus));
    };
...
const BdeDsr = () => {
    ...
    const handleCloseModel = () => {
        setOpenModel(false);
        dispatch(GetAllDSRApi(currentPage + 1, pageLength));
    };

    const handleChangePage = (event, newPage) => {
        // console.log('🚀 newPage:', newPage);
        setCurrentPage(newPage);
        dispatch(GetAllDSRApi(newPage + 1, pageLength, bdeData));
    };

    const handleChangeRowsPerPage = (event) => {
        dispatch(GetAllDSRApi(currentPage + 1, event.target.value, bdeData));
    ...
    const onBdeChangeHandle = (e, newValue) => {
        ...
        dispatch(GetAllDSRApi(currentPage + 1, pageLength, selectValue));
