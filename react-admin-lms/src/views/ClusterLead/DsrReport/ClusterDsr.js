...
function Row({ row, key, currentPage, pageLength, leadStatus }) {
    ...
    return (
        <>
            <StyledTableRow hover sx={{ '& > *': { borderBottom: 'unset' } }} key={key} unmountOnExit>
                {/* <TableCell sx={{ pl: 3, py: 0 }}>
                    <IconButton aria-label="expand row" size="small" onClick={() => setOpen(!open)}>
                        {open ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />}
                    </IconButton>
                </TableCell> */}
                {/* <StyledTableCell component="th" scope="row" sx={{ py: 2 }}>
                    {row?.bd_name}
                </StyledTableCell>{' '}
                <StyledTableCell component="th" scope="row" sx={{ py: 2 }}>
                    {row?.bd_target}
...
const ClusterDsr = () => {
    ...
    useEffect(() => {
        dispatch(GetAllClusterDSRApi(1, pageLength, startDate, endDate));
    ...
    const handleCloseModel = () => {
        setOpenModel(false);
        dispatch(GetAllClusterDSRApi(currentPage + 1, pageLength));
    };

    const handleChangePage = (event, newPage) => {
        // console.log('🚀 newPage:', newPage);
        setCurrentPage(newPage);
        dispatch(GetAllClusterDSRApi(newPage + 1, pageLength, bdeData));
    };

    const handleChangeRowsPerPage = (event) => {
        dispatch(GetAllClusterDSRApi(currentPage + 1, event.target.value, bdeData));
