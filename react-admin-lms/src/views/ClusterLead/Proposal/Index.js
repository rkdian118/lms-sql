...
function Row({ row, key, currentPage, pageLength, search, bdeData }) {
    ...
    const handleCloseModel = () => {
        ...
        setOpenEditModel(false);
        setId('');
        dispatch(ClusterGetAllProposalApi(newCurrentPage, pageLength, search, bdeData));
    };
...
const Index = () => {
    ...
    useEffect(() => {
        dispatch(ClusterGetAllProposalApi(1, pageLength, '', bdeData));
    }, [pageLength]);
    ...
    const handleCloseModel = () => {
        setOpenModel(false);
        dispatch(ClusterGetAllProposalApi(currentPage + 1, pageLength, search, bdeData));
    };
    const onSearchLead = (e) => {
        if (e.target.value.length > 2) {
            dispatch(ClusterGetAllProposalApi(currentPage + 1, pageLength, e.target.value, bdeData));
        }
        if (e.target.value.length === 0) {
            dispatch(ClusterGetAllProposalApi(currentPage + 1, pageLength, '', bdeData));
        }
    ...
    const handleChangePage = (event, newPage) => {
        ...
        dispatch(ClusterGetAllProposalApi(newPage + 1, pageLength, '', bdeData));
    };

    const handleChangeRowsPerPage = (event) => {
        dispatch(ClusterGetAllProposalApi(currentPage + 1, event.target.value, '', bdeData));
    ...
    const onBdeChange = (e, newValue) => {
        ...
        dispatch(ClusterGetAllProposalApi(currentPage + 1, pageLength, search, selectValue, bdeData));
    };
    return (
        <MainCard
            content={false}
