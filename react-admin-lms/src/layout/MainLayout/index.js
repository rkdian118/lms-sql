...
import NewSidebar from './Sidebar/NewIndex';
...
const MainLayout = () => {
    ...
    const { adminRole, adminDetails } = useSelector((state) => state.admin);
    ...
    return (
        <Box sx={{ display: 'flex' }}>
            ...
            {/* {adminRole === 'Cluster' ? '' : <Sidebar />} */}

            {/* {adminRole === 'Cluster' ? <Sidebar /> : <>{adminRole === 'Business' ? <NewSidebar /> : <Sidebar />}</>} */}

            {adminRole === 'Business' ? <Sidebar /> : <Sidebar />}
            {/* <Sidebar /> */}
            {/* main content */}
