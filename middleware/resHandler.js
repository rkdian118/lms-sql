...
const { createUserLogsService } = require("../service/ManagerService/commonTableService");
...
const errorMiddleware  = (err,req,res,next) =>{
  // console.log("Error Middleware: ",err.statusCode, err.message, err.success, err?.error?.name, err?.error?.code, err.error);
    ...
    if (err?.error?.name === 'MongooseError' || err?.error?.name === 'MongoError') {
...
const SuccessHandler = async (req, res, message, status=200, data = null) =>{
  ...
  // const userLog = await createUserLogsService(req, res, message, status, data);
  const success = status == 207 ? false : true;
  return res.status(status).json({ "ResponseCode": status, "ResponseMessage": message, "succeeded": success, "ResponseData": data });
