...
const {
  ...
  getBranchessDropdownService,
} = require("../../service/AdminService/adminLeadService");

class LeadsControllers {
  async getLeadsControllers(req, res, next) {
    try {
      const result = await getLeads(req);
  ...
  async getBranchesDropdownControllers(req, res, next) {
    try {
      const result = await getBranchessDropdownService(req);
      const { success, msg, status, data } = result;
      if (success) {
        SuccessHandler(req, res, msg, status, data);
      } else {
