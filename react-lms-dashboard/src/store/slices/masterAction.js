...
const slice = createSlice({
    ...
    reducers: {
        ...
        Projection_Status_List_Success(state, action) {
            state.projectionStatusList = action.payload?.ResponseData;
            state.projectionStatusLoading = false;
        }
    }
});
...
export const ProjectionStatusGetApi = () => async () => {
    try {
        // const response = await axios.get(`${PROXY}get-call-type`);
        const response = await axiosApiHelper('get', `${PROXY}get-master-status?status_type=projection_status`);
        if (response.data.succeeded === true) {
            dispatch(slice.actions.Projection_Status_List_Success(response.data));
        }
        return response.data;
    } catch (error) {
        // dispatch(slice.actions.Login_Failed(error));
