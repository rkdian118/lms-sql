...
import { GetLeadDetailsDataApi } from 'store/slices/businessAction';
...
const LeadDetailModel = (props) => {
    const { open, close, leadId, GetLeadDetailsDataApi } = props;
    const dispatch = useDispatch();
    const fileRef = useRef(null);
    const { getLeadDetailsData, getLeadDetailsLoading } = useSelector((state) => state.businessAction);
    // console.log('🚀  leadId:', getLeadDetailsData);
    const [loader, setLoader] = useState(false);
    const [value, setValue] = useState('');

    useEffect(() => {
        GetLeadDetailsDataApi(leadId);
    }, [leadId]);
...
export default connect(null, { GetLeadDetailsDataApi })(LeadDetailModel);
