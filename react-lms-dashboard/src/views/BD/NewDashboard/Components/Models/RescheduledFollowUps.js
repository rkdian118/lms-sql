...
import { RescheduledFollowUpsApi } from 'store/slices/businessAction';
...
const RescheduledFollowUps = (props) => {
    ...
    const { open, close, RescheduledFollowUpsApi, followUpsId, activityId } = props;
    ...
    const onSubmit = (values, { setSubmitting }) => {
        const newData = {
            ...
            followup_date: moment(values.selectedDate).startOf('day'),
            // followup_date: moment(new Date(values.selectedDate).setHours(0, 0, 0, 0)),
            comment: `Follow-Up Reschedule - ${values.new_Comment}`
        };
        // console.log('🚀  newData:');

        RescheduledFollowUpsApi(newData)
            .then((res) => {
...
export default connect(null, { RescheduledFollowUpsApi })(RescheduledFollowUps);
