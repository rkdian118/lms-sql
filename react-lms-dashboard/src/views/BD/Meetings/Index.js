...
import { CallsTableLoader } from 'ui-component/cards/Skeleton/MuiTableLoader';
...
function Row({ row, key, currentPage, pageLength, search, leadId, callStatusId, startDate, endDate }) {
    ...
    const handleCloseModel = () => {
        ...
        setId('');
        dispatch(GetAllCallsApi(newCurrentPage, pageLength, search, leadId, callStatusId, startDate, endDate));
    };
...
const Index = () => {
    ...
    const { getCallsList, getCallsLoading } = newmenu;
    ...
    useEffect(() => {
        dispatch(GetAllCallsApi(currentPage + 1, pageLength, search, leadId, callStatusId, startDate, endDate));
    ...
    const onOk = async (selectedDate) => {
        ...
        await dispatch(GetAllCallsApi(currentPage + 1, pageLength, search, leadId, callStatusId, start, end));
    ...
    const onLeadChange = (e, newValue) => {
        ...
        dispatch(GetAllCallsApi(currentPage + 1, pageLength, search, selectValue, callStatusId, startDate, endDate));
    };
    ...
    const onCallStatusChange = (e, newValue) => {
        ...
        dispatch(GetAllCallsApi(currentPage + 1, pageLength, search, leadId, selectValue, startDate, endDate));
    };
    // console.log('🚀leadId:', leadId);
    const handleChangePage = (event, newPage) => {
        // console.log('🚀 newPage:', newPage);
        setCurrentPage(newPage);
        dispatch(GetAllCallsApi(newPage + 1, pageLength, search, leadId, callStatusId, startDate, endDate));
    };

    const handleChangeRowsPerPage = (event) => {
        dispatch(GetAllCallsApi(currentPage + 1, event.target.value, search, leadId, callStatusId, startDate, endDate));
    ...
    const onSearchLead = (e) => {
        if (e.target.value.length > 2) {
            dispatch(ClearMeetingData());
            dispatch(GetAllCallsApi(currentPage + 1, pageLength, e.target.value, leadId, callStatusId, startDate, endDate));
        }
        if (e.target.value.length === 0) {
            dispatch(ClearMeetingData());
            dispatch(GetAllCallsApi(currentPage + 1, pageLength, '', leadId, callStatusId, startDate, endDate));
        }
        setSearch(e.target.value);
