...
import { FilterTargetsApi } from 'store/slices/businessAction';
...
const Index = ({ FilterTargetsApi }) => {
    const theme = useTheme();
    const [currentPage, setCurrentPage] = useState(0);
    const [pageLength, setPageLength] = useState(10);
    const [openPopupModel, setOpenModel] = useState(false);
    const [search, setSearch] = useState('');
    const { getFilterTargetDetails, getFilterTargetDetailsLoading } = useSelector((state) => state.businessAction);
    ...
    return (
        <>
            <MainCard
                ...
                title={
                    <Stack direction="row">
                        <IconTargetArrow /> &nbsp; BDE Targets
                    </Stack>
                }
                sx={{ border: '0px solid', padding: '5px' }}
            >
                <Grid container spacing={2} sx={{ '&.MuiGrid-root': { pt: 0 } }}>
                    <Grid item xs={12} md={12} sx={{ alignItems: 'left' }}>
                        <form onSubmit={formik.handleSubmit}>
                            <Grid container spacing={gridSpacing} sx={{ mt: 1, px: 2 }}>
                                ...
                                <Grid
                                    ...
                                    xs={12}
                                    md={4}
                                    sx={{ mb: 0, display: 'flex', alignItems: 'center', justifyContent: 'flex-start' }}
                                >
                    ...
                    <Grid item xs={12} md={12} sx={{ mx: 1, mb: 2, alignItems: 'left' }}>
                        {/* {usersResult} */}
                        <Grid container spacing={2} sx={{ mt: 0 }}>
                            {!getFilterTargetDetailsLoading && getFilterTargetDetails?.length > 0 ? (
                                getFilterTargetDetails?.map((list, i) => {
                                    ...
                                    return (
                                        <Grid key={i} item xs={6} md={6} sx={{ mb: 0, alignItems: 'left' }}>
                                            <Card
                                                sx={{
                                                    p: 2,
                                                    background:
...
export default connect(null, { FilterTargetsApi })(Index);
