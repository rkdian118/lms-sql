...
import { GetTargetCardLoader } from 'ui-component/cards/Skeleton/BranchLoader';
import AnimateButton from 'ui-component/extended/AnimateButton';
import { ViewTargetLoader } from 'ui-component/cards/Skeleton/AdminLoader';
...
const ViewTargets = ({ FilterTargetsApi }) => {
    ...
    useEffect(() => {
        // dispatch(GetBranchSelectedDropDownApi());
    ...
    const onSubmit = (values, { setSubmitting }) => {
        const newData = {
            ...
            isBranchTarget: values.isBranchTarg,
    ...
    useEffect(() => {
        const newData = {
            isCLTarget: formik.values.isClusterTarg,
            isBranchTarget: formik.values.isBranchTarg,
    ...
    //     dispatch(FilterTargetsApi(currentPage + 1, pageLength, selectValues));
    // };
                                ...
                                <Grid container spacing={gridSpacing} sx={{ mt: 1 }}>
                                    ...
                                    <Grid item xs={12} sx={{ mb: 0, alignItems: 'left' }}>
                                        <Autocomplete
                                            multiple
                                            fullWidth
                                            options={getBranchDropdownSelectedList}
                                            getOptionLabel={(option) => option.name}
                                            value={getBranchDropdownSelectedList?.filter((option) =>
                                                formik.values.selectedBranch.includes(option._id)
                                            )}
                                            onChange={handleCLChange}
                                            renderInput={(params) => (
                                                <TextField
                                                    {...params}
                                                    label="Select Branch Head"
                                                    inputProps={{
                                                        ...params.inputProps,
                                                        autoComplete: 'off'
                                                    }}
...
export default connect(null, { FilterTargetsApi })(ViewTargets);
