...
import { BDTargetsTableLoader } from 'ui-component/cards/Skeleton/AdminLoader';
...
const Index = () => {
    const { branchTargetList, branchTargetLoading } = useSelector((state) => state.adminAction);
    ...
    useEffect(() => {
        dispatch(BranchTargetGetApi(currentPage + 1, pageLength, selectedMonth, FormatYear, bdeData));
    }, [pageLength]);

    const handleChangePage = (event, newPage) => {
        // console.log('🚀 newPage:', newPage);
        setCurrentPage(newPage);
        dispatch(BranchTargetGetApi(newPage + 1, pageLength, selectedMonth, FormatYear, bdeData));
    };

    const handleChangeRowsPerPage = (event) => {
        dispatch(BranchTargetGetApi(currentPage + 1, event.target.value, selectedMonth, FormatYear, bdeData));
    ...
    const handleEditModel = (data) => {
        ...
        // dispatch(BranchTargetGetApi(currentPage + 1, pageLength));
    };
    ...
    const handleMonthChange = (event, newValue) => {
        ...
        dispatch(BranchTargetGetApi(currentPage + 1, pageLength, months, FormatYear, bdeData));
    };
    ...
    const onBdeChange = (e, newValue) => {
        ...
        dispatch(BranchTargetGetApi(currentPage + 1, pageLength, selectedMonth, FormatYear, selectValue));
    };
    ...
    return (
        <>
            <MainCard
                ...
                title={
                    <Stack direction="row">
                        <TrackChangesIcon sx={{ mr: 2 }} /> &nbsp; Branch Target Based On Month
                    </Stack>
                }
            ...
            >
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {!branchTargetLoading && branchTargetList?.docs?.length > 0 ? (
                                branchTargetList?.docs?.map((row, i) => (
                                    <>
                                        ...
                                        <TableRow hover key={i} unmountOnExit>
                                            ...
                                            <TableCell sx={{ py: 1 }}>{row?.target_month}</TableCell>
                                            <TableCell sx={{ py: 1 }}>{row?.target_year}</TableCell>
                                            <TableCell sx={{ py: 1 }}>{row?.cluster_head?.name}</TableCell>
                ...
                <TablePagination
                    ...
                    component="div"
                    // rows={rows}
                    count={branchTargetList.totalDocs}
