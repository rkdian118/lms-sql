...
import { LeadProposalStatusGetApi } from 'store/slices/masterAction';
...
import AddCallMode from './AddLeadProposalStatus';
import EditCallMode from './EditLeadProposalStatus';

const LeadProposalStatusList = ({ LeadProposalStatusGetApi }) => {
    const { leadProposalStatusList, leadProposalStatusLoading } = useSelector((state) => state.masterAction);
    const [openModel, setOpenModel] = useState(false);
    const [id, setId] = useState('');
    const [name, setName] = useState('');

    useEffect(() => {
        LeadProposalStatusGetApi();
    }, []);
    ...
    return (
        <>
            ...
            {!leadProposalStatusLoading && leadProposalStatusList.length > 0 ? (
                leadProposalStatusList.map((list, i) => (
            ...
            {openModel && <AddCallMode open={openModel} close={() => handleCloseModel()} />}
            {id && openModel && <EditCallMode open={openModel} close={() => handleCloseModel()} id={id} getName={name} />}
        </>
    );
};

export default connect(null, { LeadProposalStatusGetApi })(LeadProposalStatusList);
