...
import { PreDefineNotesGetApi } from 'store/slices/masterAction';
...
import AddPreDefine from './AddPreDefine';
import EditPreDefine from './EditPreDefine';

const PreDefineNotesList = ({ PreDefineNotesGetApi }) => {
    const { preDefineNotesList, preDefineNotesLoading } = useSelector((state) => state.masterAction);
    ...
    return (
        <>
            ...
            {!preDefineNotesLoading && preDefineNotesList.length > 0 ? (
            ...
            {openModel && <AddPreDefine open={openModel} close={() => handleCloseModel()} />}
            {id && openModel && <EditPreDefine open={openModel} close={() => handleCloseModel()} id={id} getName={name} />}
        </>
    );
};

export default connect(null, { PreDefineNotesGetApi })(PreDefineNotesList);
