...
const LeadStatusList = ({ LeadStatusGetApi }) => {
    const theme = useTheme();
    const { leadStatusList, leadStatusLoading } = useSelector((state) => state.masterAction);
            ...
            >
                <TableContainer>
                    <Table aria-label="simple table">
                        ...
                        <TableBody>
                            {!leadStatusLoading && leadStatusList.length > 0 ? (
                ...
                {openModel && (
                    <AddMasterStatus statusType="lead_status" name="Lead Status" open={openModel} close={() => handleCloseModel()} />
                )}
                {id && openEditModel && (
                    <EditMasterStatus
                        open={openEditModel}
                        close={() => handleCloseModel()}
                        statusType="lead_status"
                        color={color}
                        name="Lead Status"
                        id={id}
                        getName={name}
...
export default connect(null, { LeadStatusGetApi })(LeadStatusList);
