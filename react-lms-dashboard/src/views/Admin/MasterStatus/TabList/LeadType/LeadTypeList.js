...
import { LeadTypeGetApi } from 'store/slices/masterAction';
import {
    Grid,
    Box,
    Chip,
...
const LeadSourceList = ({ LeadTypeGetApi }) => {
    const theme = useTheme();
    const { leadTypeList, leadTypeLoading } = useSelector((state) => state.masterAction);
            ...
            >
                <TableContainer>
                    <Table aria-label="simple table">
                        ...
                        <TableBody>
                            {!leadTypeLoading && leadTypeList.length > 0 ? (
                ...
                {openModel && <AddMasterStatus statusType="lead_type" name="Lead Type" open={openModel} close={() => handleCloseModel()} />}
                {id && openEditModel && (
                    <EditMasterStatus
                        open={openEditModel}
                        close={() => handleCloseModel()}
                        color={color}
                        statusType="lead_type"
                        name="Lead Type"
                        id={id}
                        getName={name}
                    />
...
export default connect(null, { LeadTypeGetApi })(LeadSourceList);
