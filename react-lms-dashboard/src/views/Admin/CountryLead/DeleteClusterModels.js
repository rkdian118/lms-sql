...
import { CountryLeadGetApi, CountryLeadStatusChangeApi } from 'store/slices/adminAction';
...
const DeleteClusterModels = ({ open, close, deleteData, currentPage, pageLength, CountryLeadStatusChangeApi }) => {
    ...
    const onDeleteStatus = () => {
        setIsLoading(true);
        const newData = { country_lead_id: deleteData._id, status: '3' };
        CountryLeadStatusChangeApi(newData)
            .then((res) => {
                setIsLoading(false);
                close();
...
export default connect(null, { CountryLeadStatusChangeApi })(DeleteClusterModels);
