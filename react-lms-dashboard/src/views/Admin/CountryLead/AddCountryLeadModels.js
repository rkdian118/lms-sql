...
import { ActiveBranchApi, AddCountryLeadApi } from 'store/slices/adminAction';
import AnimateButton from 'ui-component/extended/AnimateButton';
import { continentalData, countries } from 'Helper/CountriesData';
...
const AddCountryLeadModels = (props) => {
    const theme = useTheme();
    const { open, close, ActiveBranchApi, AddCountryLeadApi } = props;
    ...
    const initialValues = {
        ...
        continentalCode: null
    };

    const validationSchema = Yup.object({
        continentalCode: Yup.mixed().required('Continent Field is Required'),
        selectedCountryCode: Yup.mixed().required('Country Code Field is Required'),
    ...
    const onSubmit = (values, { setSubmitting }) => {
        ...
        formData.append('continent_code', values?.continentalCode?.code);
        setLoader(true);
        // console.log('formData', formData);
        AddCountryLeadApi(formData)
            .then((res) => {
    ...
    const handleContinentalCodeChange = (event, newValue) => {
        formik.setFieldValue('continentalCode', newValue); // Set the value of the 'country' field in Formik
    };
                        ...
                        <Formik>
                            <form noValidate onSubmit={formik.handleSubmit}>
                                <Grid container spacing={2} sx={{ p: '10px 30px' }}>
                                    <Grid item xs={12} md={6} sx={{ mb: 0, alignItems: 'left' }}>
                                    ...
                                    <Grid item xs={12} md={4} sx={{ mb: 0, alignItems: 'left' }}>
                                        <Typography variant="h5" sx={{ width: '180px' }}>
                                            Continent<span style={{ color: 'red' }}>*</span>
                                        </Typography>
                                        <FormControl fullWidth>
                                            <Autocomplete
                                                // id="country-select-demo"
                                                options={continentalData}
                                                autoHighlight
                                                ...
                                                value={formik.values.continentalCode} // Set the value of Autocomplete from Formik
                                                onChange={handleContinentalCodeChange} // Handle changes and update Formik field value
                                                onBlur={formik.handleBlur}
                                                renderInput={(params) => (
                                                    <TextField
                                                        {...params}
                                                        // label="Choose a continentalCode"
                                                        label="Enter Continent Code"
                                                        onBlur={formik.handleBlur}
                                                        error={formik.touched.continentalCode && Boolean(formik.errors.continentalCode)}
                                                        helperText={formik.touched.continentalCode && formik.errors.continentalCode}
                                                        inputProps={{
                                                            ...params.inputProps,
...
export default connect(null, { ActiveBranchApi, AddCountryLeadApi })(AddCountryLeadModels);
