...
import { ActiveBranchApi, UpdateCountryLeadApi, CountryLeadGetDetailsApi } from 'store/slices/adminAction';
...
const EditClusterModels = (props) => {
    const { open, close, clusterId, CountryLeadGetDetailsApi, ActiveBranchApi, UpdateCountryLeadApi } = props;
    const { countryLeadDetails, countryLeadDetailsLoading } = useSelector((state) => state.adminAction);
    ...
    useEffect(() => {
        setName(countryLeadDetails?.name);
        setDesignation(countryLeadDetails?.designation);
        setEmail(countryLeadDetails?.email);
        setMobileNumber(countryLeadDetails?.mobile);
        // setProfilePic(BASE_URL + countryLeadDetails?.profile_pic);
        setShowProfile(BASE_URL + countryLeadDetails?.profile_pic);
    }, [countryLeadDetails, getDesignation, name, getEmail, mobileNumber, showProfile]);
    ...
    const onSubmit = (values, { setSubmitting }) => {
        ...
        formData.append('country_lead_id', clusterId);
                                ...
                                <Grid container spacing={2} sx={{ p: '10px 30px' }}>
                                    ...
                                    <Grid item xs={6} sx={{ mb: 0, alignItems: 'left' }}>
                                        {(formik.values.profile_pic && <PreviewImage file={formik.values.profile_pic || profilePic} />) ||
                                            (showProfile && (
                                                // <Grid>
                                                <img src={showProfile || profilePic} alt="preview" width="150px" height="auto" />
                                                // </Grid>
                                            ))}
                                    </Grid>
                                </Grid>
...
export default connect(null, { ActiveBranchApi, UpdateCountryLeadApi, CountryLeadGetDetailsApi })(EditClusterModels);
