...
function Row({ row, key, currentPage, pageLength, search, leadStatus, bdeData, clusterLeadData }) {
    ...
    const handleCloseModel = () => {
        ...
        dispatch(GetAllDuplicateLeadListApi(newCurrentPage, pageLength, search, leadStatus, bdeData, clusterLeadData));
    };
    ...
    return (
        <>
            <TableRow hover key={key}>
                ...
                <TableCell align="left" sx={{ py: 0 }}>
                    {row?.leadData?.lead_bd_data?.bdName ? (
                        <MuiTypography sx={{ color: '#2196f3' }}>{row?.leadData?.lead_bd_data?.bdName}</MuiTypography>
                    ) : (
                        '-'
                    )}
                </TableCell>
...
const Index = () => {
    ...
    const { getDuplicateLeadList, getDuplicateLeadLoading, getClDropdownList, getSelectedBDEDropdownList } = useSelector(
    ...
    useEffect(() => {
        dispatch(GetAllDuplicateLeadListApi(currentPage + 1, pageLength, search, leadStatus, bdeData, clusterLeadData));
    }, [pageLength]);
    ...
    const onSearchLead = (e) => {
        if (e.target.value.length > 2) {
            dispatch(GetAllDuplicateLeadListApi(currentPage + 1, pageLength, e.target.value, leadStatus, bdeData, clusterLeadData));
        }
        if (e.target.value.length === 0) {
            dispatch(GetAllDuplicateLeadListApi(currentPage + 1, pageLength, '', leadStatus, bdeData, clusterLeadData));
        }
    ...
    const handleChangePage = (event, newPage) => {
        ...
        dispatch(GetAllDuplicateLeadListApi(newPage + 1, pageLength, '', leadStatus, bdeData, clusterLeadData));
    };

    const handleChangeRowsPerPage = (event) => {
        dispatch(GetAllDuplicateLeadListApi(currentPage + 1, event.target.value, '', leadStatus, bdeData, clusterLeadData));
    ...
    const onLeadStatusChange = (e, newValue) => {
        ...
        dispatch(GetAllDuplicateLeadListApi(currentPage + 1, pageLength, '', selectValue, bdeData, clusterLeadData));
    };
    ...
    const onClusterLeadChange = (e, newValue) => {
        ...
        dispatch(GetAllDuplicateLeadListApi(currentPage + 1, pageLength, search, leadStatus, '', selectValue));
    };

    const onBdeChange = (e, newValue) => {
        const selectValue = newValue ? newValue._id : '';
        setBdeData(selectValue);
        dispatch(GetAllDuplicateLeadListApi(currentPage + 1, pageLength, search, leadStatus, selectValue, clusterLeadData));
    };
    ...
    return (
        <MainCard
            ...
            title={
                <Stack direction="row">
                    <IconBook2 sx={{ mr: 2 }} /> &nbsp; Duplicate Leads
                </Stack>
            }
            ...
            secondary={
                <Stack direction="row" spacing={2} alignItems="center">
                    ...
                    {/* <CSVExport data={getDuplicateLeadList?.docs} type={1} filename={onExcelFileName()} header={header} />
                    <CSVExport data={getDuplicateLeadList?.docs} type={2} filename={onExcelFileName()} header={header} /> */}
                    <Autocomplete
        ...
        >
            ...
            <TableContainer>
                <Table aria-label="collapsible table">
                    ...
                    <TableBody>
                        {!getDuplicateLeadLoading && getDuplicateLeadList?.docs?.length > 0 ? (
                            getDuplicateLeadList?.docs?.map((row, i) => (
                                <Row
                                    row={row}
                                    key={i}
                        ...
                        {/* {getDuplicateLeadList.docs.map((row) => (
            ...
            <TablePagination
                ...
                component="div"
                // rows={rows}
                count={getDuplicateLeadList.totalDocs}
