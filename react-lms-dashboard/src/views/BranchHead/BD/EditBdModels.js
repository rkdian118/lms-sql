...
import { UpdateBDApi, BDGetDetailsApi, ClusterLeadDropDownApi } from 'store/slices/clusterAction';
...
const EditBdModels = (props) => {
    const theme = useTheme();
    const { open, close, UpdateBDApi, BDGetDetailsApi, ClusterLeadDropDownApi, bdId } = props;
    const dispatch = useDispatch();
    const fileRef = useRef(null);
    const { bdeDetailsDataList, bdeDetailsDataLoading } = useSelector((state) => state.clusterAction);
    ...
    useEffect(() => {
        ClusterLeadDropDownApi().then((res) => {
            setTeamLeadData(res?.ResponseData);
        });
        // BDGetDetailsApi(bdId).then((res) => {
        setTeamLead(bdeDetailsDataList?.cluster_lead_id?._id ? bdeDetailsDataList?.cluster_lead_id?._id : '');
        setName(bdeDetailsDataList?.name);
        setDesignation(bdeDetailsDataList?.designation);
        setEmail(bdeDetailsDataList?.email);
        setMobileNumber(bdeDetailsDataList?.mobile);
        setShowProfile(BASE_URL + bdeDetailsDataList?.profile_pic);
        setContryCode(bdeDetailsDataList.country_code ? bdeDetailsDataList.country_code : null);
        // });
    }, [bdeDetailsDataList, getDesignation, name, getEmail, mobileNumber, showProfile, contryCode]);
    ...
    const initialValues = {
        ...
        profile_pic: '',
        clusterLead_id: teamLead,
        selectedCountryCode: null || countries.find((country) => country.phone === bdeDetailsDataList?.country_code)
    };
    ...
    const onSubmit = (values, { setSubmitting }) => {
        ...
        formData.append('_id', bdId);
        formData.append('name', values.bd_name);
        formData.append('email', values.email);
        formData.append('mobile', values.mobile);
        formData.append('country_code', values?.selectedCountryCode?.phone);
        formData.append('designation', values.designation);
        formData.append('cluster_lead_id', values.clusterLead_id);
        if (values.password) {
                                ...
                                <Grid container spacing={2} sx={{ p: '10px 30px' }}>
                                    <Grid item xs={6} sx={{ mb: 0, alignItems: 'left' }}>
                                        ...
                                        <TextField
                                            ...
                                            name="bd_name"
                                            value={formik.values.bd_name}
                                            onChange={formik.handleChange}
                                            error={formik.touched.bd_name && Boolean(formik.errors.bd_name)}
                                            helperText={formik.touched.bd_name && formik.errors.bd_name}
                                    ...
                                    <Grid item xs={6} sx={{ mb: 0, alignItems: 'left' }}>
                                        ...
                                        <FormControl fullWidth>
                                            <Select
                                                fullWidth
                                                name="clusterLead_id"
                                                value={formik.values.clusterLead_id ? formik.values.clusterLead_id : ''}
                                                onChange={formik.handleChange}
                                                error={formik.touched.clusterLead_id && Boolean(formik.errors.clusterLead_id)}
                                                helperText={formik.touched.clusterLead_id && formik.errors.clusterLead_id}
                                                onBlur={formik.handleBlur}
                                                displayEmpty
                                                style={{
                                                    display: 'flex',
                                            ...
                                            {formik.touched.clusterLead_id && formik.errors.clusterLead_id && (
                                                <FormHelperText error>{formik.errors.clusterLead_id}</FormHelperText>
                                            )}
                                        </FormControl>
                                    </Grid>
...
export default connect(null, { UpdateBDApi, ClusterLeadDropDownApi, BDGetDetailsApi })(EditBdModels);
