...
import { ClusterUpdateProposalApi } from 'store/slices/clusterAction';
...
const EditProposalModels = (props) => {
    ...
    const { open, close, ClusterUpdateProposalApi, rowData } = props;
    const dispatch = useDispatch();
    const { proposalTypeList } = useSelector((state) => state.masterAction);
    const { getLeadDropdown } = useSelector((state) => state.clusterAction);
    ...
    const initialValues = {
        ...
        proposal_type: '' || rowData?.proposal[0]?.proposal_type_id,
        requirement_type: '' || rowData?.leadData?.reqTypeData?.status_name,
        comment_remark: '' || rowData?.proposal[0]?.comment_remark,
    ...
    const onSubmit = (values, { setSubmitting }) => {
        ...
        ClusterUpdateProposalApi(formData)
            .then((res) => {
...
export default connect(null, { ClusterUpdateProposalApi })(EditProposalModels);
