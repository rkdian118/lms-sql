...
import { GetRfpActivityDetailsApi, AddCommentRfpActivityApi } from 'store/slices/businessAction';
...
const RFPActivityStatus = (props) => {
    const theme = useTheme();
    const { open, close, AddCommentRfpActivityApi, rowData, callById } = props;
    const newmenu = useSelector((state) => state.countryLeadAction);
    const { getRfpActivityData, getRfpActivityLoading } = newmenu;
    ...
    useEffect(() => {
        if (getRfpActivityData?.length > 0) {
            setCommentData(getRfpActivityData[0]);
            setActivityId(getRfpActivityData[0]?._id);
        }
    }, [getRfpActivityData]);

    // console.log('🚀commentData:', activityId);
    ...
    const handleUpdateActivityStatus = () => {
        ...
        dispatch(GetRfpActivityDetailsApi(callById));
    };
    const initialValues = {
        new_Comment: ''
    };
    ...
    const onSubmit = (values, { setSubmitting }) => {
        const newData = {
            rfp_activity_id: activityId,
            comment: values.new_Comment
        };

        AddCommentRfpActivityApi(newData)
            .then((res) => {
                                        ...
                                        <CardContent sx={{ py: 2, pr: 0 }}>
                                            ...
                                            >
                                                {!getRfpActivityLoading && getRfpActivityData?.length > 0 ? (
                                                    getRfpActivityData?.map((tab, index) => (
                                                        <Tab
                                                            key={index}
                                                            label={
                                                                ...
                                                                <Grid container spacing={1}>
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <Typography
                                                                            ...
                                                                            color={theme.palette.mode === 'dark' ? 'dark.900' : ''}
                                                                        >
                                                                            {moment(tabcontent?.createdAt).format('Do MMM YYYY')}
                                                                        </Typography>
                                                                    </Grid>
                                                                </Grid>
                                                            </CardContent>
...
export default connect(null, { AddCommentRfpActivityApi })(RFPActivityStatus);
