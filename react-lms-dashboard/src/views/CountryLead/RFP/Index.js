...
// import { ClusterGetAllLeadDropDownApi, ClusterGetAllBdeDropdownApi } from 'store/slices/clusterLeadAction';
...
import { GetRfpTableLoader } from 'ui-component/cards/Skeleton/BranchLoader';
...
function Row({ row, key, currentPage, pageLength, search, bdeData, clusterLeadData }) {
    ...
    const handleCloseModel = () => {
        ...
        setOpenStatus(false);
        setId('');
        dispatch(ClusterGetAllRfpRequestApi(newCurrentPage, pageLength, search, bdeData, clusterLeadData));
...
const Index = () => {
    ...
    const newmenu = useSelector((state) => state.countryLeadAction);
    const { getRfpList, getRfpLoading, getClDropdownList, getSelectedBDEDropdownList } = newmenu;
    ...
    useEffect(() => {
        dispatch(ClusterGetAllRfpRequestApi(1, pageLength, search, bdeData, clusterLeadData));
    }, [pageLength]);
    ...
    const handleCloseModel = () => {
        setOpenModel(false);
        dispatch(ClusterGetAllRfpRequestApi(currentPage + 1, pageLength, search, bdeData, clusterLeadData));
    };
    const onSearchLead = (e) => {
        if (e.target.value.length > 2) {
            dispatch(ClusterGetAllRfpRequestApi(currentPage + 1, pageLength, e.target.value, bdeData, clusterLeadData));
        }
        if (e.target.value.length === 0) {
            dispatch(ClusterGetAllRfpRequestApi(currentPage + 1, pageLength, '', bdeData, clusterLeadData));
        }
    ...
    const handleChangePage = (event, newPage) => {
        ...
        dispatch(ClusterGetAllRfpRequestApi(newPage + 1, pageLength, search, bdeData, clusterLeadData));
    };

    const handleChangeRowsPerPage = (event) => {
        dispatch(ClusterGetAllRfpRequestApi(currentPage + 1, event.target.value, search, bdeData, clusterLeadData));
    ...
    const onBdeChange = (e, newValue) => {
        ...
        dispatch(ClusterGetAllRfpRequestApi(currentPage + 1, pageLength, search, selectValue, clusterLeadData));
    };
