...
import { ClusterUpdateRfpActivityApi } from 'store/slices/countryLeadAction';
...
const UpdateRfpActivityModels = (props) => {
    const theme = useTheme();
    const { open, close, ClusterUpdateRfpActivityApi, activityId } = props;
    ...
    const validationSchema = Yup.object({
        rfp_status_id: Yup.string().required('RFP Status Field is Required'),
    ...
    const onSubmit = (values, { setSubmitting }) => {
        const newData = {
            ...
            activity_id: values.rfp_status_id,
            comment: values.new_Comment
        };

        ClusterUpdateRfpActivityApi(newData)
            .then((res) => {
                                ...
                                <Grid container spacing={2} sx={{ p: '10px 30px' }}>
                                    <Grid item xs={12} sx={{ mb: 0, alignItems: 'left' }}>
                                        <Typography variant="h5" sx={{ width: '180px' }}>
                                            RFP Status<span style={{ color: 'red' }}>*</span>
                                        </Typography>
                                        <Select
                                            fullWidth
                                            name="rfp_status_id"
...
export default connect(null, { ClusterUpdateRfpActivityApi })(UpdateRfpActivityModels);
