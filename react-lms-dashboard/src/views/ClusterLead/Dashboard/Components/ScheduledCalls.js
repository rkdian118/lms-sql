...
function Row({ row, key, currentPage, pageLength, startDate, endDate, bdeData }) {
    ...
    const onCloseCallStatusModel = () => {
        ...
        dispatch(ClusterGetAllCallsDataApi(startDate, endDate, bdeData, newCurrentPage, pageLength));
    };
                ...
                <TableCell align="left" sx={{ py: 1 }}>
                    {row?.callData?.call_status ? (
                        <Chip
                            ...
                            variant="outlined"
                            sx={{
                                color: getCallStatusColor(row?.callData?.call_status),
                                borderColor: getCallStatusColor(row?.callData?.call_status),
                                cursor: 'pointer',
                                fontSize: '0.875rem'
                            }}
...
const ScheduledCalls = ({ ClusterGetAllCallsDataApi, CallStatusGetApi, startDate = '', endDate = '', bdeData = '' }) => {
    const theme = useTheme();
    ...
    // useEffect(() => {
    //     ClusterGetAllCallsDataApi(startDate, endDate, bdeData, currentPage + 1, pageLength);
    ...
    useEffect(() => {
        if (bdeData === '' || startDate === '' || endDate === '') {
            ...
            ClusterGetAllCallsDataApi(startDate, endDate, bdeData, 1, pageLength);
            setCurrentPage(0);
        }
    ...
    const handleChangePage = (event, newPage) => {
        ...
        dispatch(ClusterGetAllCallsDataApi(startDate, endDate, bdeData, newPage + 1, pageLength));
    };

    const handleChangeRowsPerPage = async (event) => {
        await ClusterGetAllCallsDataApi(startDate, endDate, bdeData, currentPage + 1, event?.target.value);
