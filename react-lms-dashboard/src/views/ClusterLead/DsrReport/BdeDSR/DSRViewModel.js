...
const DSRViewModel = ({ open, close, dsrId }) => {
    ...
    console.log('🚀 ~ DSRViewModel ~ getDSRDetails:', getDSRDetails);

    const handleClose = () => {
        close();
    };

    // console.log('getDSRDetailsLoading', getDSRDetails);
    return (
        <div>
            <BootstrapDialog
                open={open}
            ...
            >
                <BootstrapDialogTitle id="customized-dialog-title" onClose={handleClose}>
                    <Typography
                        ...
                        component="h6"
                        align="center"
                        sx={{ pl: '10px', fontSize: '1em', display: 'flex', justifyContent: 'center' }}
                    >
                        {getDSRDetailsLoading ? (
                            <Skeleton variant="rectangular" height={20} width={200} sx={{ my: 0.5, align: 'center' }} />
                        ) : (
                            <>{getDSRDetails?.bd_id?.name} DSR</>
                        )}
                    </Typography>
                    <Typography
                        variant="h6"
                ...
                <PerfectScrollbar component="div" style={{ scrollbarWidth: 'thin', scrollbarColor: '#b2b3b3 #f1f1f1', maxWidth: '100%' }}>
                    <DialogContent sx={{ '&.MuiDialogContent-root': { p: '0' } }} dividers>
                        <CardWrapper>
                            {!getDSRDetailsLoading ? (
                                <Grid container spacing={gridSpacing}>
                                    <Grid item xs={6} sx={{ borderRight: '0px solid #5bb4fe', '&.MuiGrid-root': { pt: '5px' } }}>
                                        <MainCard content={false} sx={{ background: 'transparent' }}>
                                            ...
                                            <FeedsCard
                                                ...
                                                count={getDSRDetails?.linkedin_messages}
                                                iconPrimary={<LinkedInIcon />}
                                                messages={2}
                                                dsrId={getDSRDetails?._id}
                                                bgColor={theme.palette.success.main}
