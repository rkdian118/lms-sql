...
function Row({ row, key, currentPage, pageLength, search, bdeData, startDate, endDate, rfpStatusId }) {
    ...
    const handleCloseModel = () => {
        ...
        setOpenStatus(false);
        setId('');
        dispatch(ClusterGetAllRfpRequestApi(newCurrentPage, pageLength, search, bdeData, startDate, endDate, rfpStatusId));
...
const Index = () => {
    ...
    useEffect(() => {
        dispatch(ClusterGetAllRfpRequestApi(currentPage + 1, pageLength, search, bdeData, startDate, endDate, rfpStatusId));
    }, [pageLength, startDate, endDate, rfpStatusId]);
    ...
    const onOk = async (selectedDate) => {
        ...
        await dispatch(ClusterGetAllRfpRequestApi(currentPage + 1, pageLength, search, bdeData, start, end, rfpStatusId));
    ...
    const onSearchLead = (e) => {
        if (e.target.value.length > 2) {
            dispatch(ClusterGetAllRfpRequestApi(currentPage + 1, pageLength, e.target.value, bdeData, startDate, endDate, rfpStatusId));
        }
        if (e.target.value.length === 0) {
            dispatch(ClusterGetAllRfpRequestApi(currentPage + 1, pageLength, '', bdeData, startDate, endDate, rfpStatusId));
        }
    ...
    const handleChangePage = (event, newPage) => {
        ...
        dispatch(ClusterGetAllRfpRequestApi(newPage + 1, pageLength, search, bdeData, startDate, endDate, rfpStatusId));
    };

    const handleChangeRowsPerPage = (event) => {
        dispatch(ClusterGetAllRfpRequestApi(currentPage + 1, event.target.value, search, bdeData, startDate, endDate, rfpStatusId));
    ...
    const onBdeChange = (e, newValue) => {
        ...
        dispatch(ClusterGetAllRfpRequestApi(currentPage + 1, pageLength, search, selectValue, startDate, endDate, rfpStatusId));
    };

    const onRfpStatusChange = (e, newValue) => {
        const selectValue = newValue ? newValue._id : '';
        setRfpStatusId(selectValue);
        dispatch(ClusterGetAllRfpRequestApi(currentPage + 1, pageLength, search, bdeData, startDate, endDate, selectValue));
    };
    return (
        <MainCard
            content={false}
                ...
                <Grid item xs={12} sm={6} md={6} lg={3}>
                    <Autocomplete
                        ...
                        renderInput={(params) => (
                            <TextField
                                {...params}
                                label="Select RFP Activity Status"
                                inputProps={{
                                    ...params.inputProps,
