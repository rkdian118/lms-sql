...
import { ClearDateRangeFilterSetup } from 'store/slices/commonAction';
...
const NavItem = ({ item, level, parentId, isParents = false }) => {
    ...
    const tabName = capitalizeFirstLetter(item?.title?.props?.id);
            ...
            {layout === LAYOUT_CONST.VERTICAL_LAYOUT || (layout === LAYOUT_CONST.HORIZONTAL_LAYOUT && matchDownMd) ? (
                ...
                >
                    {drawerOpen ? (
                        ...
                        <MuiTooltip title={tabName} arrow placement="right">
                            <ButtonBase aria-label="theme-icon" sx={{ borderRadius: `${borderRadius}px` }} disableRipple={drawerOpen}>
                                <ListItemIcon
                                    sx={{
                                        minWidth: level === 1 ? 36 : 18,
