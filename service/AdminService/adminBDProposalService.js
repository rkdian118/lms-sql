...
class AdminBdProposalService {

    async adminUpdateProposalService(req) {
        try {
    ...
    async adminGetProposalService(req) {
        try {
            const { page, limit, search, lead_id, bd_id, cluster_lead_id, cluster_head_id, is_dashboard, } = req.query;
            const options = {
            ...
            const resultAggregate = proposalModel.aggregate([
                ...
                },
                {
                    $group: {
                        ...
                        upfront_amount: { $first: "$upfront_amount", },
                        project_amount: { $first: "$project_amount", },
                        proposal_submitted: { $first: "$proposal_submitted", },
                        timeline_days: { $first: "$timeline_days", },
                        man_hour: { $first: "$man_hour", },
...
module.exports = new AdminBdProposalService();
