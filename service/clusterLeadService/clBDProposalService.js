...
class BdProposalService {
    // async clAddProposalService(req) {
    //     try {
    ...
    async clUpdateProposalService(req) {
        try {
            const { body, files, user } = req;
            ...
            if (!checkProposal) {
                return { success: false, msg: "proposal not found.", status: 400, data: checkProposal };
            }

            let checklead = await leadModel
                .findOne({
                    $and: [
            ...
            body.project_name != "" && checkProposal.project_name != body.project_name ? checkProposal.project_name = body.project_name : checkProposal.project_name = checkProposal.project_name;
            body.upfront_amount != "" && checkProposal.upfront_amount != body.upfront_amount ? checkProposal.upfront_amount = body.upfront_amount : checkProposal.upfront_amount = checkProposal.upfront_amount;
            body.project_amount != "" && checkProposal.project_amount != body.project_amount ? checkProposal.project_amount = body.project_amount : checkProposal.project_amount = checkProposal.project_amount;
            body.proposal_submitted != "" && checkProposal.proposal_submitted != body.proposal_submitted ? checkProposal.proposal_submitted = body.proposal_submitted : checkProposal.proposal_submitted = checkProposal.proposal_submitted;
            body.timeline_days != "" && checkProposal.timeline_days != body.timeline_days ? checkProposal.timeline_days = body.timeline_days : checkProposal.timeline_days = checkProposal.timeline_days;
            body.man_hour != "" && checkProposal.man_hour != body.man_hour ? checkProposal.man_hour = body.man_hour : checkProposal.man_hour = checkProposal.man_hour;

            const data = await checkProposal.save();
            if (data) {
                body.project_amount != "" && checklead.project_amount != body.project_amount ? checklead.project_amount = body.project_amount : checklead.project_amount = checklead.project_amount;
            ...
            return { status: 201, success: true, msg: "Proposal update successfully.", data: data };
        } catch (error) {
            return { status: 500, success: false, msg: "Something went wrong when add proposal", data: error };
        }
    }

    async clGetProposalService(req) {
        try {
            const { page, limit, search, lead_id, bd_id, proposal_type_id, start_date, end_date } = req.query;
            const options = {
            ...
            proposal_type_id ? proposalTypeFilter['proposal.proposal_type_id'] = new mongoose.Types.ObjectId(proposal_type_id) : "";
            ...
            if (!proposals || proposals == "") {
                return { status: 201, success: true, msg: "Proposals not found.", data: [] };
            }
            return { status: 200, success: true, msg: "Proposals get successfully.", data: proposals };
        } catch (error) {
            return {
                status: 400,
                success: false,
