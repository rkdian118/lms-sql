...
const {
  assignManagerTargetService,
} = require("../ClusterService/clusManTargService");
...
class CLBDTargtService {
  async clGetBDsTargetService(req) {
    const { page, limit, search, bd_id, targetYear, targetMonth } = req.query;
    const { selectedBDE, selectedMonth, selectedYear, isClusterTarg } =
      req.body;
    const options = {
    ...
    if (isClusterTarg) {
      ...
      req?.user?._id
        ? filterTarg.push({
          "clusterLeadTargData.cluster_lead_id": new mongoose.Types.ObjectId(
            req?.user?._id
          ),
        })
        : "";
      ...
      selectedBDE && selectedBDE.length > 0
        ...
        : filterBDs.push({ cluster_lead_id: req.user._id });
      ...
      req?.user?._id
        ? filterTarg.push({
          "bdTargData.cluster_lead_id": new mongoose.Types.ObjectId(
            req?.user?._id
          ),
        })
        : "";
  ...
  async clGetBDTargetDetailService(req, bd_id, target_id) {
    // const { _id } = req.query;
  ...
  async clUpdateBDTargetService(req) {
    try {
      const body = req.body;
      ...
      if (bdTarget.target_month == currentMonth) {
        ...
        if (updatebdTarget) {
          ...
          if (clusterLeadTarg) {
            clusterLeadTarg.assigned_targets =
              clusterLeadTarg.assigned_targets + addTarg;
            let updateclusterLeadTarg = await clusterLeadTarg.save();
          }

          const managerTarg = await managerTargetModel.findOne({
            $and: [
              { manager_id: updatebdTarget.manager_id },
              { target_month: currentMonth },
            ],
          });
          if (managerTarg) {
            managerTarg.assigned_targets =
              managerTarg.assigned_targets + addTarg;
      ...
      return {
        status: 400,
        success: false,
        msg: "Somethin went wrong when update BDE target service.",
        data: error,
      };
    }
  }
  ...
  async BDTargetDropdownService(req) {
    try {
      const matchObject = {};
      matchObject.cluster_lead_id = req.user._id;

      const dropdownTarget = await bdTargetModel.aggregate([
        {
          $match: matchObject,
        },
        {
      ...
      const dropDownBDE = await bdModel.find(matchObject).select("_id name designation")

      const concatArray = await dropdownTarget.concat(dropDownBDE)
  ...
  // async clManCompBDTargetService(req) {
  //   try {
  ...
  //       status: 500,
  //       success: false,
  //       msg: "Something went wrong when Complete BDE Target service.",
  //       data: error,
  //     };
  //   }
  // };

  // async clAssignBDTargetService(req) {
  //   try {
  ...
  //       status: 500,
  //       success: true,
  //       msg: "Assign Team Lead target service error.",
  //       data: error,
  //     };
  //   }
  // }

  // async clCheckManagerTargets(manager_id, year, month) {
  //   try {
...
//       status: 500,
//       success: true,
//       msg: "Update BD's targets to Team Lead service error.",
//       data: error,
//     };
//   }
// }
module.exports = new CLBDTargtService();
