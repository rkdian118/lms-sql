...
class CLBdRfpService {
  async clGetLeadRFPTypeService(req) {
    try {
  ...
  async clGetLeadRFPStatusService(req) {
    try {
  ...
  async clAddRFPService(req) {
    try {
      const { body, files, user } = req;

      let checkLead = await leadModel.findById(new mongoose.Types.ObjectId(body.lead_id))
      if (!checkLead) {
        return {
          success: false,
          msg: "Lead not found.",
      ...
      const rfp = await rfpModel
        .findOne({
          $and: [{ bd_id: checkLead.bd_id }, { lead_id: new mongoose.Types.ObjectId(body.lead_id) }],
        })
          ...
          comment: [
            {
              ...
              comment: data?.comment_remark
                ? data.comment_remark
                : `Cluster Lead ${rfpActivityData.status_name}`,
            },
          ],
        });
  ...
  async clUpdateRFPProposalValueService(req) {
    try {
      const { body, files, user } = req;
      ...
      body.proposal_value != "" && checkRfp.proposal_value != body.proposal_value ? checkRfp.proposal_value = body.proposal_value : checkRfp.proposal_value;
      const data = await checkRfp.save();
  ...
  async clUpdateRfpStatusService(req) {
    try {
      ...
      return { status: 201, success: true, msg: "RFP status updated successfully.", data: updaterfpStatus };
  ...
  async clGetRFPService(req) {
    try {
      const { page, limit, search, lead_id, bd_id, start_date, end_date, rfp_status_id } = req.query;
      const options = {
      ...
      rfp_status_id ? userFilter.push({ lead_rfp_status: new mongoose.Types.ObjectId(rfp_status_id) }) : "";
      ...
      if (!rfp || rfp == "") {
        return {
          status: 201,
          success: true,
          msg: "Proposals requests not found.",
          data: [],
        };
      }
      return {
  ...
  async clGetRFPActivitiesService(req) {
    try {
  ...
  async clUpdateRFPActivityService(req) {
    try {
      ...
      if (data) {
        ...
        let rfpUpdate = await rfpModel.updateOne(
          ...
          {
            $set: { "rfp.$.activity_id": new mongoose.Types.ObjectId(data?.activity_id) }
          }
        );
  ...
  async clAddCommentRFPActivityService(req) {
    try {
...
module.exports = new CLBdRfpService();
