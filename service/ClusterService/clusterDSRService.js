...
class ClBdDSRService {
  ...
  async getCLDSRReportService(req) {
    try {
      const { page, limit, search, start_date, end_date, cluster_lead_id } =
        req.query;
      const options = {
        page: page ? page : 1,
        limit: limit ? limit : 20,
  ...
  async getClusterDSRReportService(req) {
    try {
  ...
  async getLeadDataBankService(req) {
    try {
      const { bh_id, start_date, end_date, lead_status, bd_id } = req.query;
      const matchObj = {};
      if (bh_id) {
        matchObj["clusterHeadData._id"] = new mongoose.Types.ObjectId(bh_id);
      }
      if (bd_id) matchObj["bdData._id"] = new mongoose.Types.ObjectId(bd_id);
      //  matchObj._id = new mongoose.Types.ObjectId("65d874cf0783772cf22425f5");

      let startDate = new Date(moment().startOf("year"));
      ...
      matchObj.assign_date = {
        $gte: startDate,
        $lt: endDate,
      };
      if (lead_status) {
        matchObj["lead_status_data.status_name"] = {
          $in: lead_status.split(","),
        };
      }
      ...
      const result = await leadModel.aggregate([
        ...
        {
          $lookup: {
            from: "master_statuses", // Replace with the actual name of your lead_activity collection
            localField: "lead_req_type_id",
            foreignField: "_id", // Assuming lead_id is the field in lead_activity that corresponds to _id in leadModel
            as: "lead_req_type_data",
          },
        },
        {
          $unwind: {
            path: "$lead_req_type_data",
            preserveNullAndEmptyArrays: true,
          },
        },
        {
        ...
        {
          $unwind: {
            path: "$lead_status_data",
            preserveNullAndEmptyArrays: true,
          },
        },
        {
          $lookup: {
            from: "businesses", // Replace with the actual name of your lead_activity collection
            localField: "bd_id",
            foreignField: "_id", // Assuming lead_id is the field in lead_activity that corresponds to _id in leadModel
            as: "bdData",
          },
        ...
        {
          $lookup: {
            from: "cluster_leads", // Replace with the actual name of your lead_activity collection
        ...
        {
          $lookup: {
            from: "clusters", // Replace with the actual name of your lead_activity collection
        ...
        {
          $lookup: {
            from: "branches", // Replace with the actual name of your lead_activity collection
            localField: "clusterHeadData.branch_id",
            foreignField: "_id", // Assuming lead_id is the field in lead_activity that corresponds to _id in leadModel
            as: "branchData",
          },
        },
        ...
        {
          $lookup: {
            from: "lead_activities", // Replace with the actual name of your lead_activity collection
            localField: "_id",
            foreignField: "lead_id", // Assuming lead_id is the field in lead_activity that corresponds to _id in leadModel
            as: "activityData",
          },
        },
        {
        ...
        },
        {
          $lookup: {
            from: "master_statuses", // Replace with the actual name of your lead_activity collection
            localField: "lastActivity.activity_id",
            foreignField: "_id", // Assuming lead_id is the field in lead_activity that corresponds to _id in leadModel
            as: "activityStatusData",
          },
        },
        {
          $unwind: {
            path: "$activityStatusData",
            preserveNullAndEmptyArrays: true,
          },
        },
        {
      ...
      const headers = [
        ...
        "Shared Project Amount (USD)",
        "Shared Upfront Amount (USD)",
        "Shared Working Timeline",
        "Proposal Comment Remark",
      ];
      ...
      result.forEach(async (item) => {
        // user.s_no = counter;
        // let proposalType = "";
        // if (item?.proposalsData) {
        //   const proposalMaster = await masterStatusModel.findById(
        //     item?.proposalsData?.proposal[0]?.proposal_type_id
        //   );
        //   proposalType = proposalMaster.status_name;
        // }
        ...
        worksheet.addRow([
          ...
          item?.assign_date,
          item?.bdData.name,
          item?.branchData?.branch_name,
          item?.clusterLeadData?.name,
          item?.clusterHeadData?.name,
          item?.lead_req_type_data?.status_name,
          item?.lead_type_data?.status_name,
          item?.lead_source_data?.status_name,
          item?.client_name,
          item?.company_name,
          item?.client_number,
          item?.client_whatsapp_num,
          item?.client_email,
          item?.client_alternate_email,
          item?.skype_id,
          item?.client_country,
          item?.address,
          item?.client_linkedin,
          item?.upwork_job_url,
          item?.bid_url,
          item?.client_budget,
          item?.proposal_amount,
          item?.lead_status_data?.status_name,
          item?.add_notes,
          item?.activityStatusData?.status_name,
          item?.lastActivity?.createdAt,
          item?.followupCalls?.length,
          item?.proposalsData?.project_name,
          // proposalType,
          item?.proposalsData?.project_amount,
          item?.proposalsData?.upfront_amount,
          item?.proposalsData?.timeline_days
            ? item?.proposalsData?.timeline_days + " days"
            : "",
          item?.proposalsData?.proposal[0]?.comment_remark,
