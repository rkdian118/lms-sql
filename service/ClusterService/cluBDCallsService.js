...
class ClusterBdCallService {
  async clusterGetLeadCallTypeService(req) {
    try {
  ...
  async clusterGetLeadCallStatusService(req) {
    try {
  ...
  async clusterGetLeadCallsService(req) {
    try {
      const {
        page,
        limit,
      ...
      cluster_lead_id
        ? userFilter.push({
            "clusterLeadData._id": new mongoose.Types.ObjectId(cluster_lead_id),
          })
        : "";
      lead_id
        ? userFilter.push({
      ...
      call_status_id
        ? userFilter.push({
            call_status_id: new mongoose.Types.ObjectId(call_status_id),
          })
        : "";
      call_type_id
        ? userFilter.push({
            call_type_id: new mongoose.Types.ObjectId(call_type_id),
          })
        : "";
  ...
  async clusterUpdateCallService(req) {
    try {
      const body = req.body;
  ...
  async clusterGetCallActivityService(req) {
    try {
  ...
  async clusterUpdateCallActivityService(req) {
    try {
      ...
      if (
        callActivity &&
        callActivity.activity_id.toString() == activity_id.toString()
      ) {
        return {
          status: 400,
          success: false,
  ...
  async clusterAddCommentCallActivityService(req) {
    try {
...
const myInstance = new ClusterBdCallService();

module.exports = new ClusterBdCallService();
